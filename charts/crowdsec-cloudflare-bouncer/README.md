...
**This chart is not maintained by the upstream project and any issues with the chart should be raised [here](https://github.com/kdlhost-charts/charts/issues/new/choose)**
...
helm repo update
helm install intel-gpu-plugin kdlhost-charts/intel-gpu-plugin
```
...
Other values may be used from the [values.yaml](https://raefon.github.io/kdlhost-charts/common/values.yaml) from the [common library](https://raefon.github.io/kdlhost-charts/).
...
```console
helm install intel-gpu-plugin kdlhost-charts/intel-gpu-plugin -f values.yaml
```
...
If your cluster runs [Node Feature Discovery](https://raefon.github.io/kdlhost-charts/charts/node-feature-discovery), you can deploy the device plugin only on nodes with Intel GPU by specifying the desired `nodeSelector` or `affinity` in your values. For example (make sure to update to your exact feature label):
...
* Upgraded `common` chart dependency to version 0.1.8
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/kdlhost-charts/intel-gpu-plugin?modal=changelog)
